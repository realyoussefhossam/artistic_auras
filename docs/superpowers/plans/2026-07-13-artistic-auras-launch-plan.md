...
- Modify: `.gitmodules` (auto-generated by `forge install`)
...
This adds the library under `lib/openzeppelin-contracts` and updates `.gitmodules`.
...
def cleanup(csv_path: Path, image_dir: Path, out_dir: Path, metadata_dir: Path) -> None:
    ...
    for row in rows:
        ...
        base, _ = Path(original_file).stem, Path(original_file).suffix
        normalized_file = f"{base}.png"
        src = image_dir / original_file
    ...
    # Simplified token list for the gallery page
    token_list = []
    for row in rows:
        token_id = int(row["tokenID"].strip())
        token_list.append(
            {
                "tokenId": token_id,
                "name": row["name"].strip(),
    ...
    with cleaned_csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["tokenID", "name", "description", "file_name", "image_cid"])
        writer.writeheader()
...
export function MintCard() {
  ...
  const remaining = maxSupply && totalSupply ? Number(maxSupply) - Number(totalSupply) : null;
  const canMint = isConnected && publicSaleActive && remaining !== null && remaining >= quantity && (canAfford ?? false);
  ...
  return (
    <Card className="w-full max-w-md">
      ...
      <CardContent className="space-y-4">
        ...
        />
        <Button onClick={handleMint} disabled={!canMint || isPending || isConfirming} className="w-full">
          {isPending || isConfirming ? "Confirming..." : "Mint"}
        </Button>
        {error && <p className="text-sm text-red-500">{error.message}</p>}
        {isSuccess && <p className="text-sm text-green-600">Mint successful!</p>}
      </CardContent>
    </Card>
  );
}
        ...
        {tokens.map((token) => (
          <Card key={token.tokenId}>
            <CardContent className="p-4 space-y-2">
              <img
                src={token.image.replace("ipfs://", "https://gateway.pinata.cloud/ipfs/")}
                alt={token.name}
